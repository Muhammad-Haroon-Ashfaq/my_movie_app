...
const getSpecficMovie = async (req, res) => {
    try {
        const {id} = req.params;
...
const movieReview = async (req, res) => {
    try {
        ...
        if (movie) {
           const alreadyReviewd = movie.reviews.find((r) => r.user.toString() === req.user._id.toString());
        ...
        movie.rating = movie.reviews.reduce((acc, item) => 
            item.rating + acc, 0) / movie.reviews.length;
